...
1) С использованием Java SDK 21 / Boot Spring 3.5.0 разработан сервис temperature_api, который на запросы /temperature?location= и /temperature/{id} отдает значение температуры в диапазоне от 10C до 30C в формате  
```markdown
{  
 "value": "number",  
 "unit": "string",  
...
``` 
2) Сервис упакован в Docker и добавлен в docker-compose. Работает на порту 8081.
3) Для сервиса smart_home в docker-compose добавлены настройки для запуска postgres с указанием скрипта инициализации ./smart_home/init.sql
Произведена проверка с использованием Postman и коллекции smarthome-api.postman_collection.json.
